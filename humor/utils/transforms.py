...
def compute_aligned_from_right(body_right):
    ...
    body_right = body_right * xy_axis  # element product getting the xy component
    world2aligned_axis = torch.cross(
        body_right, x_axis.expand_as(body_right)
    )  # ZL this just gives you the z axis....???

    world2aligned_aa = (
        world2aligned_axis
        / (torch.norm(world2aligned_axis, dim=1, keepdim=True) + 1e-6)
    ) * world2aligned_angle  # ZL: Rotation around z axis by angle. Delta z
    # this is the problem. This is the root cause of the problem. 1e-6
