...
class MotionOptimizer():
    ...
    def apply_cam2prior(self,
        ...
        if 'trans' in data_dict and 'root_orient' in data_dict:
            ...
            if inverse:
                ...
                if T > 1:
                    trans_offset = trans[np.arange(B),
                                         key_frame_idx, :].unsqueeze(1)
                else:
                    trans_offset = trans[:, 0:1, :]
                trans = trans - trans_offset
                # rotates to camera frame
                trans = torch.matmul(R_time.transpose(3, 2),
                                     trans.reshape((B, T, 3, 1)))[:, :, :, 0]
                ...
                trans = torch.matmul(R_time, trans.reshape(
                    (B, T, 3, 1)))[:, :, :, 0]
                # then apply floor offset so the root joint is at the desired height
                cur_smpl_data, _ = self.smpl_results(trans,
                                                     prior_dict['root_orient'],
                                                     body_pose, betas)
                if T > 1:
                    cur_root_height = cur_smpl_data['joints3d'][np.arange(B),
                                                                key_frame_idx,
                                                                0, 2:3]
                else:
    ...
    def estimate_velocities(self,
        ...
        if self.motion_prior.model_data_config in [
            ...
            if smpl_results is None:
                smpl_results, _ = self.smpl_results(trans, root_orient,
                                                    body_pose, betas)
            trans_vel = self.estimate_linear_velocity(trans, h)
            joints_vel = self.estimate_linear_velocity(
                smpl_results['joints3d'], h)
            root_orient_mat = batch_rodrigues(root_orient.reshape(
                (-1, 3))).reshape((B, T, 3, 3))
            root_orient_vel = self.estimate_angular_velocity(
                root_orient_mat, h)
    ...
    def infer_latent_motion(self,
        ...
        if self.motion_prior.model_data_config in [
            ...
            if self.optim_floor:
                ...
                prior_data_dict = self.apply_cam2prior(
                    data_dict, self.cam2prior_R, self.cam2prior_t,
                    self.cam2prior_root_height, body_pose, betas,
                    self.init_fidx)
                trans = prior_data_dict['trans']
                root_orient = prior_data_dict['root_orient']

            smpl_results, _ = self.smpl_results(trans, root_orient, body_pose,
                                                betas)
            ...
            if self.motion_prior.in_rot_rep == 'mat' or self.motion_prior.in_rot_rep == '6d':
                root_orient_in = batch_rodrigues(root_orient.reshape(
                    -1, 3)).reshape((B, T, 9))
                body_pose_in = batch_rodrigues(body_pose.reshape(
                    -1, 3)).reshape((B, T, J_BODY * 9))
            if self.motion_prior.in_rot_rep == '6d':
                root_orient_in = root_orient_in[:, :, :6]
                body_pose_in = body_pose_in.reshape(
                    (B, T, J_BODY, 9))[:, :, :, :6].reshape((B, T, J_BODY * 6))
            ...
            if full_forward_pass:
                ...
                if self.motion_prior.in_rot_rep != 'mat':
                    seq_dict['trans'] = batch_rodrigues(
                        root_orient.reshape(-1, 3)).reshape((B, T, 9))
                    seq_dict['pose_body'] = batch_rodrigues(
                        body_pose.reshape(-1, 3)).reshape((B, T, J_BODY * 9))
                ...
                for k in infer_results.keys():
                    if k != 'posterior_distrib' and k != 'prior_distrib':
                        infer_results[k] = infer_results[
                            k][:, :, 0]  # only want first output step
                infer_results = (seq_dict, infer_results)
            else:
                prior_z, posterior_z = infer_results
                infer_results = posterior_z[
                    0]  # mean of the approximate posterior
        else:
            raise NotImplementedError(
                'Only smpl+joints motion prior configuration is supported!')
    ...
    def rollout_latent_motion(self,
                              trans,
                              root_orient,
                              body_pose,
                              betas,
        ...
        if self.optim_floor:
            ...
            data_dict = {'trans': trans, 'root_orient': root_orient}
            prior_data_dict = self.apply_cam2prior(data_dict, self.cam2prior_R,
                                                   self.cam2prior_t,
        ...
        if self.motion_prior.model_data_config in [
            ...
            if self.motion_prior.in_rot_rep == 'mat' or self.motion_prior.in_rot_rep == '6d':
                root_orient_in = batch_rodrigues(root_orient.reshape(
                    -1, 3)).reshape((B, 1, 9))
                body_pose_in = batch_rodrigues(body_pose.reshape(
                    -1, 3)).reshape((B, 1, J_BODY * 9))
            if self.motion_prior.in_rot_rep == '6d':
                root_orient_in = root_orient_in[:, :, :6]
                body_pose_in = body_pose_in.reshape(
                    (B, 1, J_BODY, 9))[:, :, :, :6].reshape((B, 1, J_BODY * 6))
        ...
        if self.motion_prior.model_data_config in [
            ...
            root_orient_out = rotation_matrix_to_angle_axis(
                root_orient_out.reshape((-1, 3, 3))).reshape((B, Tm1, 3))
            ...
            body_pose_out = rotation_matrix_to_angle_axis(
                body_pose_out.reshape((-1, 3, 3))).reshape(
                    (B, Tm1, J_BODY * 3))
            ...
            if return_vel:
                trans_vel_out = torch.cat([trans_vel, pred_dict['trans_vel']],
                                          dim=1)
                ...
                joints_vel_out = torch.cat([
                    joints_vel, pred_dict['joints_vel'].reshape(
                        (B, Tm1, -1, 3))
                ],
                                           dim=1)
            ...
            if self.motion_prior.model_data_config == 'smpl+joints+contacts':
                ...
                # expand to full body
                full_contact_conf = torch.zeros(
                    (B, Tm1, len(SMPL_JOINTS))).to(contact_conf)
                full_contact_conf[:, :,
                                  CONTACT_INDS] = full_contact_conf[:, :,
                                                                    CONTACT_INDS] + contact_conf
                full_contacts = torch.zeros(
                    (B, Tm1, len(SMPL_JOINTS))).to(pred_contacts)
                full_contacts[:, :,
                              CONTACT_INDS] = full_contacts[:, :,
                                                            CONTACT_INDS] + pred_contacts
                # repeat first entry for t0
                full_contact_conf = torch.cat(
                    [full_contact_conf[:, 0:1], full_contact_conf], dim=1)
                full_contacts = torch.cat(
                    [full_contacts[:, 0:1], full_contacts], dim=1)
        ...
        if self.optim_floor:
            ...
            cam_dict = self.apply_cam2prior(data_dict,
                                            self.cam2prior_R,
                                            self.cam2prior_t,
                                            self.cam2prior_root_height,
                                            out_dict['pose_body'],
    ...
    def latent2pose(self, latent_pose):
        ...
        body_pose = rotation_matrix_to_angle_axis(
            body_pose.reshape((B * T * J_BODY, 3, 3))).reshape(
                (B, T, J_BODY * 3))
        return body_pose
    ...
    def pose2latent(self, body_pose):
        ...
        latent_pose = latent_pose_distrib.mean.reshape(
            (B, T, self.latent_pose_dim))
        return latent_pose
    ...
    def smpl_results(self, trans, root_orient, body_pose, beta):
        ...
        if T == 1:
            ...
            root_orient = root_orient.expand(
                (self.batch_size, self.seq_len, 3))
            body_pose = body_pose.expand(
                (self.batch_size, self.seq_len, J_BODY * 3))
        elif T != self.seq_len:
        ...
        smpl_body = self.body_model(pose_body=body_pose.reshape(
            (self.batch_size * self.seq_len, -1)),
                                    pose_hand=None,
                                    betas=betas.reshape(
                                        (self.batch_size * self.seq_len, -1)),
                                    root_orient=root_orient.reshape(
                                        (self.batch_size * self.seq_len, -1)),
                                    trans=trans.reshape(
                                        (self.batch_size * self.seq_len, -1)))
        # body joints
        joints3d = smpl_body.Jtr.reshape(
            (self.batch_size, self.seq_len, -1, 3))[:, :T]
        body_joints3d = joints3d[:, :, :len(SMPL_JOINTS), :]
        added_joints3d = joints3d[:, :, len(SMPL_JOINTS):, :]
        # ALL body vertices
        points3d = smpl_body.v.reshape(
            (self.batch_size, self.seq_len, -1, 3))[:, :T]
        ...
        pred_data = {
            ...
            'verts3d': verts3d,
            'joints3d_extra':
            added_joints3d,  # hands and selected OP vertices (if applicable) 
            'faces':
            smpl_body.f  # always the same, but need it for some losses
        }
    ...
    def zero_pad_tensors(self, pad_list, pad_size):
        ...
        for pad_idx, pad_tensor in enumerate(pad_list):
            padding = torch.zeros(
                (B, pad_size, pad_tensor.size(2))).to(pad_tensor)
