...
def train(args_obj, config_file):
    ...
    if args.ckpt is not None:
        ...
        start_epoch, min_val_loss, min_train_loss = load_state(
            args.ckpt,
            model,
            optimizer=load_optim,
            map_location=device,
            ignore_keys=model.ignore_keys)
        start_epoch += 1
        Logger.log(
            'Resuming from saved checkpoint at epoch idx %d with min val loss %.6f...'
    ...
    if args.sched_samp_start is not None and args.sched_samp_end is not None:
        if args.sched_samp_start >= 0 and args.sched_samp_end >= args.sched_samp_start:
            Logger.log(
                'Using scheduled sampling starting at epoch %d and ending at epoch %d!'
                % (args.sched_samp_start, args.sched_samp_end))
            use_sched_samp = True
        else:
            Logger.log(
    ...
    train_loader = DataLoader(train_dataset,
                              ...
                              )  # get around pytorch RNG seed bug
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if use_sched_samp:
            if epoch >= args.sched_samp_start and epoch < args.sched_samp_end:
                frac = (epoch - args.sched_samp_start) / (
                    args.sched_samp_end - args.sched_samp_start)
                sched_samp_gt_p = 1.0 * (1.0 - frac)
            elif epoch >= args.sched_samp_end:
                # autoregressive
                sched_samp_gt_p = 0.0
            Logger.log('Scheduled sampling current use_gt_p = %f' %
                       (sched_samp_gt_p))
            ...
            if args_obj.loss_dict['kl_loss_cycle_len'] > 0:
                ...
                if (epoch % args_obj.loss_dict['kl_loss_cycle_len']) > (
                        args_obj.loss_dict['kl_loss_cycle_len'] // 2):
        ...
        for i, data in enumerate(train_loader):
            ...
            try:
                ...
                optimizer.zero_grad()
                # run model
                loss, stats_dict = model_class.step(model,
                                                    loss_func,
                                                    data,
                                                    train_dataset,
                                                    device,
                ...
                if torch.isnan(total_norm):
                    Logger.log(
                        'WARNING: NaN gradients. Skipping to next data...')
                    torch.cuda.empty_cache()
                    continue
                optimizer.step()
                ...
                if epoch > 0:
                    ...
                    Logger.log(
                        'WARNING: caught an exception during forward or backward pass. Skipping to next data...'
                    )
                    Logger.log(e)
                    traceback.print_exc()
                    reset_loss_track = train_dataset.pre_batch(epoch=epoch)
        ...
        if epoch % args.save_every == 0:
            ...
            save_file = os.path.join(ckpts_path,
                                     'epoch_%08d_model.pth' % (epoch))
        ...
        if epoch % args.val_every == 0:
            with torch.no_grad():
                ...
                for i, data in enumerate(val_loader):
                    ...
                    if torch.isnan(loss):
                        Logger.log(
                            'WARNING: NaN loss on VALIDATION. Skipping to next data...'
                        )
                        continue
                    ...
                    total_elapsed_t = time.time() - train_start_t
                    stats_dict['loss'] = loss
                    stats_dict['time_per_batch'] = torch.Tensor(
                        [batch_elapsed_t])[0]

                    stat_tracker.update(stats_dict,
                                        tag='val',
                                        save_tf=(i == (len(val_loader) - 1)),
