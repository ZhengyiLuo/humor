...
def create_gif(img_path, out_path, fps):
    ...
    subprocess.run(["ffmpeg", "-y", "-i", vid_path, "-pix_fmt", "rgb8", out_path])
...
def viz_results(
    ...
    if use_offscreen:
        ...
        # or gif
        create_gif(
            os.path.join(pred_out_path + "/frame_%08d.png"), pred_out_path + ".gif", fps
        )
        create_gif(
            os.path.join(gt_out_path + "/frame_%08d.png"), gt_out_path + ".gif", fps
        )
        create_comparison_images(
            gt_out_path, pred_out_path, comparison_out_path, text1="GT", text2="Pred"
        )
        create_gif(
            os.path.join(comparison_out_path + "/frame_%08d.png"),
            comparison_out_path + ".gif",
            fps,
        )
