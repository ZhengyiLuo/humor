...
class HumorLoss(nn.Module):
    def __init__(
        ...
        smpl_losses = [
            self.smpl_joint_loss_weight,
            self.smpl_mesh_loss_weight,
    ...
    def forward(self, pred_dict, gt_dict, cur_epoch, gender=None, betas=None):
        ...
        if self.contacts_loss_weight > 0.0:
            if "contacts" in gt_dict.keys() and "contacts" in pred_dict.keys():
                ...
                print(
                    "Cannot compute contact loss without contact pred/gt! Skipping..."
                )
        ...
        if self.contacts_vel_loss_weight > 0.0:
            if "contacts" in pred_dict.keys(
                ...
                print(
                    "Cannot compute contact vel loss without contact and joints_vel pred! Skipping..."
                )
        ...
        if self.use_smpl_losses:
            if gender is None or betas is None:
                raise Exception(
                    "Must pass gender and betas to MotionVAE loss to use SMPL losses!"
                )
            ...
            try:
                ...
                print(
                    "ERROR: In order to use SMPL losses must have trans, root_orient, and pose_body in pred and gt dicts!"
                )
                exit()
            ...
            for gender_name in gender_names:
                ...
                gt_body = bm(
                    pose_body=cur_gt_pose,
                    betas=cur_betas,
                    root_orient=cur_gt_orient,
                    trans=cur_gt_trans,
                )
                if pad_size > 0:
            ...
            if self.smpl_joint_consistency_loss_weight > 0.0:
                if not "joints" in pred_dict.keys():
                    print(
                        "Must regress joints in order to use smpl joint consistency loss!"
                    )
                    exit()
