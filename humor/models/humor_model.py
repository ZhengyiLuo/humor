...
class HumorModel(nn.Module):
    ...
    def roll_out_single(
        ...
        ## Compared to roll_out, need to take in the current global position and orientation
        ...
        for k in cur_input_dict.keys():
            if k in x_pred_dict:
                # drop oldest frame and add new prediction
                # ZL: this could be the model for future forecaster
                keep_frames = cur_input_dict[k][:, 1:, :]
                # print(keep_frames.size())
        ...
        # aggregate global pred_seq
        pred_seq_out = dict()
        pred_seq_out = {k: v for k, v in cur_world_dict.items()}
        ...
        if return_prior:
            ...
            return pred_seq_out, cur_input_dict
    ...
    def sample_step(
        ...
        if t_in is not None:
            ...
            if self.use_conditional_prior:
                ...
                # use standard normal
                pm, pv = (
                    torch.zeros((B, self.latent_size)).to(past_in),
                    torch.ones((B, self.latent_size)).to(past_in),
                )
