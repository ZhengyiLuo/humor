...
from datasets.amass_utils import (
    TRAIN_DATASETS,
    TEST_DATASETS,
    VAL_DATASETS,
    SPLITS,
...
from utils.transforms import (
    batch_rodrigues,
    rot6d_to_rotmat,
    compute_world2aligned_joints_mat,
    matrot2axisangle,
)
...
class AmassDiscreteDataset(Dataset):
    ...
    def __init__(
        ...
        self.effective_seq_len = (
            ...
            + 1
            + (self.step_frames_out - 1) * self.frames_out_step_size
        )  # number of frames we need to subsample to build a data sequence
        ...
        Logger.log(
            "This split contains %d sequences (that meet the duration criteria)."
            % (self.num_seq)
        )
    ...
    def load_data(self):
        ...
        if self.split_by == "single":
            ...
            if self.split_by == "dataset":
                ...
                dataset_dirs = [
                    os.path.join(amass_root, dataset_name)
                ...
                for data_dir in dataset_dirs:
                    ...
                    if self.split_by == "dataset":
                        ...
                        if self.splits_path is not None:
                            Logger.log(
                                "Note: Using specified subject splits rather than manually splitting!"
                            )
                            # load the desired split
                            split_subject_dirs = None
                            with open(
                                os.path.join(self.splits_path, self.split + ".txt"), "r"
                            ) as f:
                            ...
                            split_subject_dirs = [
                                os.path.join(data_dir, f) for f in split_subject_dirs
                            ]
                            # make sure we actually have these subjects
                            valid_subj = [
                                subj_dir in all_subject_dirs
                                for subj_dir in split_subject_dirs
                            ]
                            ...
                            if self.split == "val":
                                split_subject_dirs = all_subject_dirs[
            ...
            for cur_subj_dir in subject_dirs:
                ...
                if self.split_by == "sequence":
                    if self.splits_path is not None:
                        Logger.log(
                            "Note: Using specified sequence splits rather than manually splitting!"
                        )
                        # load the desired split
                        ...
                        split_seq_files = [
                            os.path.join(cur_subj_dir, f) for f in split_seq_files
                        ]
                        print(split_seq_files)
                        # make sure we actually have these sequences
                        valid_seq = [
                            seq_dir in all_seq_files for seq_dir in split_seq_files
                        ]
                ...
                split_seq_files = split_seq_files[
                    split_seq_dur >= self.effective_seq_len
                ]
        ...
        subseq_map = (
            dict()
        )  # map data_idx -> (sequence_path_idx, start_frame, end_frame)
        data_idx = 0
        for seq_idx, seq_info in enumerate(sequence_info):
            seq_dur, seq_nframes, seq_fps = seq_info
            sample_nframes = (
                self.effective_seq_len
            )  # need num_frames with an input (past) and output (future) so must add 1
            start_frame_idx = 0
            ...
            while end_frame_idx <= seq_nframes:
                ...
                start_frame_idx += (
                    self.sample_num_frames
                )  # some test sequences may overlap slightly (future outputs from one sequences may appear as inputs in another sequence)
    ...
    def __getitem__(self, idx):
        ...
        sample_nframes = (
            self.effective_seq_len
        )  # need num_frames with an input (past) and output (future) and future may be at large step size
        ...
        pose_body_mat = (
            batch_rodrigues(torch.Tensor(pose_body).reshape(-1, 3))
            .numpy()
            .reshape((sample_nframes, NUM_BODY_JOINTS * 9))
        )  # T x 21 x 9
        root_orient_mat = batch_rodrigues(
            torch.Tensor(root_orient).reshape(-1, 3)
        ).numpy()  # T x 3 x 3
        ...
        if self.return_global:
            ...
            trans2joint[0, 0, :2] = -(joints[0:1, 0, :] + global_world2aligned_trans)[
                0, :2
            ]

            sidx_glob = 0 if self.only_global else 1  # return full vs only outputs
            glob_num_frames = (
                (self.sample_num_frames + 1)
                if self.only_global
                else self.sample_num_frames
            )
            eidx_glob = self.sample_num_frames + 1
            if self.return_cfg["root_orient"]:
                # root orient
                global_root_orient = np.matmul(
                    global_world2aligned_rot,
                    root_orient_mat[sidx_glob:eidx_glob].copy(),
                ).reshape((glob_num_frames, 9))
                # print(global_root_orient.shape)
                global_data_dict["global_root_orient"] = global_root_orient
            if self.return_cfg["trans"]:
                # trans
                global_trans = (
                    trans[sidx_glob:eidx_glob].copy() + global_world2aligned_trans
                )
            ...
            if self.return_cfg["joints"]:
                ...
                global_joints = joints[
                    sidx_glob:eidx_glob
                ].copy() + global_world2aligned_trans.reshape((1, 1, 3))
                global_joints += trans2joint
                global_joints = np.matmul(
                    global_world2aligned_rot, global_joints.reshape((-1, 3)).T
                ).T.reshape((glob_num_frames, len(SMPL_JOINTS), 3))
                global_joints -= trans2joint
            ...
            if self.return_cfg["verts"]:
                ...
                global_verts = np.matmul(
                    global_world2aligned_rot, global_verts.reshape((-1, 3)).T
                ).T.reshape((glob_num_frames, num_verts, 3))
                global_verts -= trans2joint
            ...
            if self.return_cfg["trans_vel"]:
                ...
                global_trans_vel = np.matmul(
                    global_world2aligned_rot, global_trans_vel.T
                ).T
            ...
            if self.return_cfg["root_orient_vel"]:
                ...
                global_root_orient_vel = np.matmul(
                    global_world2aligned_rot, global_root_orient_vel.T
                ).T
            ...
            if self.return_cfg["joints_vel"]:
                ...
                global_joints_vel = np.matmul(
                    global_world2aligned_rot, global_joints_vel.reshape((-1, 3)).T
                ).T.reshape((glob_num_frames, len(SMPL_JOINTS), 3))
            ...
            if self.return_cfg["verts_vel"]:
                ...
                global_verts_vel = np.matmul(
                    global_world2aligned_rot, global_verts_vel.reshape((-1, 3)).T
                ).T.reshape((glob_num_frames, num_verts, 3))
                # print(global_verts_vel.shape)
                global_data_dict["global_verts_vel"] = global_verts_vel
            if self.return_cfg["pose_body"]:
                global_data_dict["global_pose_body"] = pose_body_mat[
                    sidx_glob:eidx_glob
                ].copy()
            if self.return_cfg["pose_body_vel"]:
                global_data_dict["global_pose_body_vel"] = pose_body_vel[
                    sidx_glob:eidx_glob
                ].copy()
            if self.return_cfg["joints_orient_vel"]:
                global_data_dict["global_joints_orient_vel"] = (
                    joints_orient_vel[sidx_glob:eidx_glob]
                    .copy()
                    .reshape((glob_num_frames, 1))
                )
            if self.return_cfg["contacts"]:
                global_data_dict["global_contacts"] = contacts[
                    sidx_glob:eidx_glob
                ].copy()
            ...
            if self.only_global:
                ...
                # add one to beta
                meta["betas"] = meta["betas"][0:1, :].expand(
                    (glob_num_frames, meta["betas"].size(1))
                )
        ...
        if self.return_cfg["pose_body"]:
            pose_body_data = np.zeros(
                (self.sample_num_frames, slice_size, NUM_BODY_JOINTS * 9)
            )
            ...
            pose_body_out = pose_body_data[
                :, self.step_frames_in :: self.frames_out_step_size
            ]

            if self.rot_rep == "aa":
                pose_body_in = matrot2axisangle(
                    pose_body_in.reshape(
                        (
                            self.sample_num_frames,
                            self.step_frames_in * NUM_BODY_JOINTS,
                            9,
                        )
                    )
                ).reshape(
                    (self.sample_num_frames, self.step_frames_in, NUM_BODY_JOINTS * 3)
                )
            elif self.rot_rep == "6d":
                pose_body_in = pose_body_in.reshape(
                    (self.sample_num_frames, self.step_frames_in, NUM_BODY_JOINTS, 9)
                )[:, :, :, :6].reshape(
                    (self.sample_num_frames, self.step_frames_in, NUM_BODY_JOINTS * 6)
                )
        ...
        if self.return_cfg["pose_body_vel"]:
            pose_body_vel_data = np.zeros(
                (self.sample_num_frames, slice_size, NUM_BODY_JOINTS, 3)
            )
            for t in range(self.sample_num_frames):
                pose_body_vel_data[t] = pose_body_vel[t : t + slice_size].copy()
            pose_body_vel_in = pose_body_vel_data[:, : self.step_frames_in]
            pose_body_vel_out = pose_body_vel_data[
                :, self.step_frames_in :: self.frames_out_step_size
            ]
        ...
        if self.return_cfg["root_orient"]:
            root_orient_mat_seq_data = np.zeros(
                (self.sample_num_frames, slice_size, 3, 3)
            )
            for t in range(self.sample_num_frames):
                cur_align_rot = world2aligned_rot[
                    t
                ]  # transform to frame of the last input step
                alinged_root_orient_slice = np.matmul(
                    cur_align_rot, root_orient_mat[t : t + slice_size].copy()
                )
                root_orient_mat_seq_data[t] = alinged_root_orient_slice

            root_orient_mat_seq_data = root_orient_mat_seq_data.reshape(
                (self.sample_num_frames, slice_size, 9)
            )
            root_orient_in = root_orient_mat_seq_data[:, : self.step_frames_in]
            root_orient_out = root_orient_mat_seq_data[
                :, self.step_frames_in :: self.frames_out_step_size
            ]

            if self.rot_rep == "aa":
                root_orient_in = matrot2axisangle(
                    root_orient_in.reshape(
                        (self.sample_num_frames, self.step_frames_in, 9)
                    )
                ).reshape((self.sample_num_frames, self.step_frames_in, 3))
            elif self.rot_rep == "6d":
                root_orient_in = root_orient_in.reshape(
                    (self.sample_num_frames, self.step_frames_in, 9)
                )[:, :, :6]
        ...
        if self.return_cfg["root_orient_vel"]:
            ...
            for t in range(self.sample_num_frames):
                ...
                aligned_root_orient_vel = np.matmul(
                    cur_align_rot, cur_root_orient_vel_data.T
                ).T
                root_orient_vel_data[t] = aligned_root_orient_vel

            root_orient_vel_in = root_orient_vel_data[:, : self.step_frames_in]
            root_orient_vel_out = root_orient_vel_data[
                :, self.step_frames_in :: self.frames_out_step_size
            ]
        ...
        if self.return_cfg["joints"]:
            joints_data = np.zeros(
                (self.sample_num_frames, slice_size, len(SMPL_JOINTS), 3)
            )
            for t in range(self.sample_num_frames):
                ...
                cur_joints_data = (
                    cur_joints_data
                    + world2aligned_trans[t].reshape((1, 1, 3))
                    + trans2joint
                )
                cur_align_rot = world2aligned_rot[t]
                aligned_cur_joints_data = np.matmul(
                    cur_align_rot, cur_joints_data.reshape((-1, 3)).T
                ).T.reshape((slice_size, len(SMPL_JOINTS), 3))
        ...
        if self.return_cfg["joints_orient_vel"]:
            ...
            for t in range(self.sample_num_frames):
                joints_orient_vel_data[t] = (
                    joints_orient_vel[t : t + slice_size]
                    .copy()
                    .reshape((slice_size, 1))
                )

            joints_orient_vel_in = joints_orient_vel_data[:, : self.step_frames_in]
            joints_orient_vel_out = joints_orient_vel_data[
                :, self.step_frames_in :: self.frames_out_step_size
            ]
        all_data_dict["joints_orient_vel"] = (
            joints_orient_vel_in,
            joints_orient_vel_out,
        )
        ...
        if self.return_cfg["verts"]:
            ...
            for t in range(self.sample_num_frames):
                ...
                cur_align_rot = world2aligned_rot[t]
                aligned_cur_verts_data = np.matmul(
                    cur_align_rot, cur_verts_data.reshape((-1, 3)).T
                ).T.reshape((slice_size, num_verts, 3))
                # back to align with global translation. Note, we do not need to rotate the offset because the global rotation is actually done about the root joint
        ...
        if self.return_cfg["joints_vel"]:
            ...
            for t in range(self.sample_num_frames):
                ...
                aligned_joint_vel = np.matmul(
                    cur_align_rot,
                    cur_joint_vel_data.reshape((slice_size * len(SMPL_JOINTS), 3)).T,
                ).T.reshape((slice_size, len(SMPL_JOINTS), 3))
                joint_vel_data[t] = aligned_joint_vel

            joints_vel_in = joint_vel_data[:, : self.step_frames_in]
            joints_vel_out = joint_vel_data[
                :, self.step_frames_in :: self.frames_out_step_size
            ]
        ...
        if self.return_cfg["verts_vel"]:
            ...
            for t in range(self.sample_num_frames):
                ...
                aligned_vert_vel = np.matmul(
                    cur_align_rot,
                    cur_vert_vel_data.reshape((slice_size * num_verts, 3)).T,
                ).T.reshape((slice_size, num_verts, 3))
                vert_vel_data[t] = aligned_vert_vel

            verts_vel_in = vert_vel_data[:, : self.step_frames_in]
            verts_vel_out = vert_vel_data[
                :, self.step_frames_in :: self.frames_out_step_size
            ]
        ...
        if self.return_cfg["contacts"]:
            contacts_data = np.zeros(
                (self.sample_num_frames, slice_size, len(CONTACT_INDS))
            )
            for t in range(self.sample_num_frames):
                contacts_data[t] = contacts[t : t + slice_size].copy()
            contacts_in = contacts_data[:, : self.step_frames_in]
            contacts_out = contacts_data[
                :, self.step_frames_in :: self.frames_out_step_size
            ]
        ...
        for k, v in all_data_dict.items():
            if self.return_cfg[k]:
                ...
                if self.noise_std > 0.0 and self.split == "train":
                    cur_in += np.random.normal(
                        loc=0.0, scale=self.noise_std, size=cur_in.shape
                    )
...
if __name__ == "__main__":
    ...
    male_bm = BodyModel(bm_path=male_bm_path, num_betas=16, batch_size=slice_size).to(
        device
    )
    male_bm_world = BodyModel(
        bm_path=male_bm_path, num_betas=16, batch_size=sample_num_frames + 1
    ).to(device)
    male_bm_global = BodyModel(
        bm_path=male_bm_path,
        num_betas=16,
        batch_size=sample_num_frames,
        use_vtx_selector=False,
    ).to(device)
    female_bm_path = os.path.join("./body_models/smplh", "female/model.npz")
    female_bm = BodyModel(
        bm_path=female_bm_path, num_betas=16, batch_size=slice_size
    ).to(device)
    female_bm_world = BodyModel(
        bm_path=female_bm_path, num_betas=16, batch_size=sample_num_frames + 1
    ).to(device)
    female_bm_global = BodyModel(
        bm_path=female_bm_path,
        num_betas=16,
        batch_size=sample_num_frames,
        use_vtx_selector=False,
    ).to(device)
    ...
    for i, data in enumerate(loader):
        ...
        if data_return_config == "smpl+joints+contacts":
            ...
            if data_rot_rep == "mat":
                root_orient_in = matrot2axisangle(
                    root_orient_in.numpy().reshape(
                        (batch_size * sample_num_frames, steps_in, 9)
                    )
                ).reshape((batch_size, sample_num_frames, steps_in, 3))
                root_orient_in = torch.Tensor(root_orient_in)
            root_orient_out_mat = data_out["root_orient"]
            root_orient_out_aa = matrot2axisangle(
                root_orient_out_mat.numpy().reshape(
                    (batch_size * sample_num_frames, steps_out, 9)
                )
            ).reshape((batch_size, sample_num_frames, steps_out, 3))
            root_orient = torch.cat(
                [root_orient_in, torch.Tensor(root_orient_out_aa)], axis=2
            ).to(device)
            ...
            if data_rot_rep == "mat":
                pose_body_in = matrot2axisangle(
                    pose_body_in.numpy().reshape(
                        (batch_size * sample_num_frames, NUM_BODY_JOINTS * steps_in, 9)
                    )
                ).reshape(
                    (batch_size, sample_num_frames, steps_in, NUM_BODY_JOINTS * 3)
                )
                pose_body_in = torch.Tensor(pose_body_in)
            pose_body_out_mat = data_out["pose_body"]
            pose_body_out_aa = matrot2axisangle(
                pose_body_out_mat.numpy().reshape(
                    (batch_size * sample_num_frames, NUM_BODY_JOINTS * steps_out, 9)
                )
            ).reshape((batch_size, sample_num_frames, steps_out, NUM_BODY_JOINTS * 3))
            ...
            root_orient_vel = torch.cat(
                [root_orient_vel_in, root_orient_vel_out], axis=2
            ).to(device)
        ...
        for t in range(0, sample_num_frames):
            ...
            if data_return_config == "smpl+joints+contacts":
                body = bm(
                    pose_body=pose_body[0, t],
                    pose_hand=None,
                    betas=betas[0, :slice_size],
                    root_orient=root_orient[0, t],
                    trans=trans[0, t],
                )
        ...
        root_orient = matrot2axisangle(
            root_orient.numpy().reshape((batch_size, sample_num_frames, 9))
        ).reshape((batch_size, sample_num_frames, 3))
        root_orient = torch.Tensor(root_orient).to(device)

        pose_body = data_out["global_pose_body"]
        pose_body = matrot2axisangle(
            pose_body.numpy().reshape(
                (batch_size * sample_num_frames, NUM_BODY_JOINTS, 9)
            )
        ).reshape((batch_size, sample_num_frames, NUM_BODY_JOINTS * 3))
        ...
        if return_config_dict["contacts"]:
            ...
            print(contacts.size())
            viz_contacts = torch.zeros(
                (batch_size, sample_num_frames, len(SMPL_JOINTS))
            ).to(contacts)
        ...
        body = bm_global(
            ...
            pose_hand=None,
            betas=betas[0, 0].reshape((1, -1)).expand((sample_num_frames, 16)),
